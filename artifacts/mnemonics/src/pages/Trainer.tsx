...
import { ChevronLeft, Timer, Brain, CheckCircle2, XCircle } from "lucide-react";
...
export default function Trainer() {
  ...
  const finishRecall = () => {
    ...
    if (content?.wordList) {
      ...
      correctCount = answers.reduce((acc, ans, i) => acc + (ans.trim().toLowerCase() === content.wordList![i].toLowerCase() ? 1 : 0), 0);
    } else if (content?.numberList) {
      totalCount = content.numberList.length;
      correctCount = answers.reduce((acc, ans, i) => acc + (ans.trim() === content.numberList![i].toString() ? 1 : 0), 0);
    } else if (content?.pairs) {
      totalCount = content.pairs.length;
      correctCount = answers.reduce((acc, ans, i) => acc + (ans.trim().toLowerCase() === content.pairs![i].back.toLowerCase() ? 1 : 0), 0);
    }
    ...
    <div className="space-y-8 pb-16 max-w-3xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        ...
        {(state === "memorize" || state === "recall") && (
          <div className="flex items-center gap-2 text-xl font-mono bg-card px-4 py-2 rounded-lg border shadow-sm">
            ...
            <Card className="border-none shadow-md bg-card/50">
              <CardContent className="p-12 text-center space-y-6">
                ...
                <p className="text-muted-foreground max-w-md mx-auto">
                  ...
                  Когда будете готовы, нажмите кнопку, чтобы перейти к вводу по памяти.
                </p>
              ...
              <CardContent className="p-8">
                <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
                  ...
                  {content.numberList?.map((num, i) => (
                    <div key={i} className="flex items-center gap-4 bg-muted/30 p-4 rounded-lg border">
                      <span className="text-muted-foreground font-mono w-6">{i + 1}.</span>
                      <span className="font-mono text-xl font-bold">{num}</span>
                    </div>
                  ))}
          ...
          <motion.div key="recall" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
            <Card className="border-none shadow-md overflow-hidden">
              ...
              <CardFooter className="p-6 bg-muted/10 border-t justify-end gap-4">
                <Button variant="outline" onClick={() => setState("memorize")}>Подсмотреть</Button>
